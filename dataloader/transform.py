...
class MyTransform(object):
    ...
    def __init__(self, reshape_size=None, crop_size=None, augment=True):
        ...
        self.count = 0
    def __call__(self, input,img2, target):
        ...
        if self.augment:
            input, img2, target = RandomCrop(self.crop_size)(input,img2,target) # RandomCrop for  image and label in the same area
            input, img2, target = self.flip(input,img2,target)               # RandomFlip for both croped image and label
            input, img2, target = self.rotate(input,img2,target)
        else:
            input,img2, target = CenterCrop(self.crop_size)(input, img2,target) # CenterCrop for the validation data
...
class Transform_test(object):
    ...
    def __call__(self, input,img2, target):
        ...
        return input_tensor, img2_tensor,target, input,img2
