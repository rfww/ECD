...
class mlDifNet_enc(nn.Module):
    ...
    def __init__(self, num_classes):
        ...
        self.final5 = nn.Conv2d(512, num_classes, 3, padding=1)
        self.final4 = nn.Conv2d(256, num_classes, 3, padding=1)
        self.final3 = nn.Conv2d(128, num_classes, 3, padding=1)
    ...
    def forward(self, x, y):
        ...
        enc5ad = abs(enc5 - enc5y)
        enc4ad = abs(enc4 - enc4y)
        enc3ad = abs(enc3 - enc3y)
        enc2ad = abs(enc2 - enc2y)
        enc1ad = abs(enc1 - enc1y)

        #return F.upsample_bilinear(self.final(enc1xy), x.size()[2:])
        enc5xy_res = F.upsample_bilinear(self.final5(enc5ad), x.size()[2:])
        enc4xy_res = F.upsample_bilinear(self.final4(enc4ad), x.size()[2:])
        enc3xy_res = F.upsample_bilinear(self.final3(enc3ad), x.size()[2:])
        enc2xy_res = F.upsample_bilinear(self.final2(enc2ad), x.size()[2:])
        enc1xy_res = F.upsample_bilinear(self.final1(enc1ad), x.size()[2:])
