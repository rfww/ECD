...
class feature_extractor(nn.Module):
    def __init__(self, backbone='', pretrained=True, freeze_backbone=False):
        ...
        self.enc5 = SegNetEnc(2048, 512, 2,1)#20
        self.enc4 = SegNetEnc(1536, 512, 2,1)#40
        self.enc3 = SegNetEnc(1024, 256, 2,1)#80
        self.enc2 = SegNetEnc(512, 128, 2,1)#160
    ...
    def forward(self, x, y,dp):
        if dp[0][0].item()>dp[0][1].item():
            c_f1,c_f2,c_f3,c_f4,c_f5 = self.resnet(x)
            n_f1,_,_,_,_ = self.resnet(y)
        else:
            c_f1,c_f2,c_f3,c_f4,c_f5 = self.resnet(y)
            n_f1,_,_,_,_ = self.resnet(x)
