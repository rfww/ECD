...
class MyTransform(object):
    ...
    def __init__(self,reshape_size=None, crop_size = None , augment=True): 
        ...
        self.count = 0
    def __call__(self, input,img2, mask, target):
        ...
        if self.reshape_size is not None:
            ...
            mask = mask.resize(self.reshape_size, Image.BILINEAR)
            target = target.resize(self.reshape_size,Image.NEAREST)
 
        if self.augment :
            input, img2, mask, target = RandomCrop(self.crop_size)(input,img2, mask, target) # RandomCrop for  image and label in the same area
            input,img2, mask, target = self.flip(input,img2, mask, target)               # RandomFlip for both croped image and label
            input,img2, mask, target = self.rotate(input,img2, mask, target)
        else:
            input,img2, mask, target =  CenterCrop(self.crop_size)(input, img2, mask, target) # CenterCrop for the validation data
        ...
        mask = ToTensor()(mask)
        Normalize([.485, .456, .406], [.229, .224, .225])(mask)
        target = torch.from_numpy(np.array(target)).long().unsqueeze(0)

        return input, img2, mask, target
...
class Transform_test(object):
    ...
    def __call__(self, input,img2, mask, target):
        ...
        Normalize([.485, .456, .406], [.229, .224, .225])(mask_tensor)
        return input_tensor, img2_tensor, mask_tensor, target, input,img2, mask
