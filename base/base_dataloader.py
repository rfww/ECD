...
class DataPrefetcher(object):
    ...
    def preload(self):
        try:
            self.next_input, self.next_input2, self.next_e_target , self.next_target = next(self.loaditer)
        ...
        with torch.cuda.stream(self.stream):
            ...
            self.next_input2 = self.next_input2.cuda(device=self.device, non_blocking=True)
            self.next_e_target = self.next_e_target.cuda(device=self.device, non_blocking=True)
    ...
    def __iter__(self):
        ...
        while self.next_input is not None:
            ...
            e_target = self.next_e_target
            target = self.next_target
            self.preload()
            count += 1
            yield input, input2, e_target, target
